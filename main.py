...
class AONet:
    ...
    def train(self, output_dir='EX-0'):
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            val_fscore, video_scores = self.eval(epoch, self.test_keys, 'results/breakfast_results.h5')
            if max_val_fscore < val_fscore:
                max_val_fscore = val_fscore
                max_val_fscore_epoch = epoch
                max_val_fscore_data = video_scores

            avg_loss = np.array(avg_loss)
            loss_mean = np.mean(avg_loss[:, 0])

            losses.append(loss_mean)
            fscores.append(max_val_fscore)

            print("   Train loss: {0:.05f}".format(loss_mean), end='')
            print('   Test F-score avg/max: {0:0.5f}/{1:0.5f}'.format(val_fscore, max_val_fscore))
        ...
        # log best result for test data
        h5_res = h5py.File('res.h5', 'a')
        for vd in max_val_fscore_data:
            max_key = vd[1]
            max_imp_scores = vd[2]
            max_machine_summary = vd[3]
            max_fm = vd[4]
            d = self.get_data(max_key)

            gt = d['gtscore'][...]
            gs = d['gtsummary'][...]
            uss = d['user_summary'][...]
            positions = d['picks'][...]
            h5_res.create_dataset(max_key + '/importance_scores', data=max_imp_scores)
            h5_res.create_dataset(max_key + '/machine_summary', data=max_machine_summary)
            h5_res.create_dataset(max_key + '/gtsummary', data=gs)
            h5_res.create_dataset(max_key + '/gtscore', data=gt)
            h5_res.create_dataset(max_key + '/user_summary', data=uss)
            h5_res.create_dataset(max_key + '/fm', data=max_fm)
            h5_res.create_dataset(max_key + '/picks', data=positions)

            video_name = max_key.split('/')[1]
            if 'video_name' in d:
                video_name = d['video_name'][...]
            h5_res.create_dataset(max_key + '/video_name', data=video_name)
        h5_res.close()

        return max_val_fscore, max_val_fscore_epoch, losses, fscores


    def eval(self, epoch, keys, results_filename):
        ...
        f_score, video_scores = self.eval_summary(epoch, summary, keys, metric=self.dataset_name,
                    results_filename=results_filename, att_vecs=att_vecs)

        return f_score, video_scores

    def eval_summary(self, epoch, machine_summary_activations, test_keys, results_filename=None, metric='tvsum', att_vecs=None):
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            importance_scores, machine_summary = generate_summary(probs, cps, num_frames, nfps, positions)
            ...
            # Reporting & logging
            video_scores.append([key_idx + 1, key, importance_scores, machine_summary, fm])
...
def train(hps):
    ...
    for split_filename in hps.splits:
        print("split_filename", split_filename)
        ...
        for split_id in range(n_folds):
            ...
            fscore, fscore_epoch, losses, fscores_list = ao.train(output_dir=hps.output_dir)
            f_avg += fscore

            losses_across_folds.append(losses)
            fscores_across_folds.append(fscores_list)
